...
export default function Customers({ onCustomerSelect, onCustomerDelete }) {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3001/api/customers/users"
      );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `http://localhost:3001/api/customers/customer?id=${id}`
      );
  ...
  return (
    <TableContainer component={Paper} className="mt-6 !rounded-2xl">
      <Table>
        <TableHead>
          <TableRow>
            <TableCell className="!font-bold !text-[14px] !uppercase ">
              User ID
            </TableCell>
            <TableCell className="!font-bold !text-[14px] !uppercase ">
              User Name
                ...
                <TableCell>
                  <div className="flex gap-6 justify-center">
                    <Button
                      ...
                      onClick={() =>
                        onCustomerSelect({ id: row.user_id, email: row.email })
                      }
                      startIcon={<Edit />}
                    >
                      Edit
                    ...
                    <Button
                      ...
                      onClick={() =>
                        onCustomerDelete({ id: row.user_id, email: row.email })
                      }
                      startIcon={<Delete />}
                    >
                      Delete
